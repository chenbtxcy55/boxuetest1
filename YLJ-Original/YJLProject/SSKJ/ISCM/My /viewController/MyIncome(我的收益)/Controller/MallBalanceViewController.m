...
/*
 商城余额
 */
#import "MallBalanceViewController.h"
...
#define kPageSize 30
@interface MallBalanceViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation MallBalanceViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.title = SSKJLocalized(@"商城余额", nil);
    [self tableView];
}
...
{
    if (!_tableView) {
        _tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, ScaleW(5), Screen_Width, ScreenHeight - Height_NavBar - ScaleW(5)) style:(UITableViewStylePlain)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell layoutYuE];
    cell.oneLab.text = dic[@"memo"];
    cell.twoLab.text = [WLTools noroundingStringWith:[dic[@"price"] doubleValue] afterPointNumber:4];
    cell.threeLab.text = [WLTools noroundingStringWith:[dic[@"nprice"] doubleValue] afterPointNumber:4];
    cell.fourLab.text = [WLTools convertTimestamp: [dic[@"addtime"] doubleValue] andFormat:@"yyyy-MM-dd"];
...
-(void)requstListrequset
{
    ...
    [[WLHttpManager shareManager]requestWithURL_HTTPCode:kIscm_get_shop_balance_Api RequestType:RequestTypePost Parameters:pamas Success:^(NSInteger statusCode, id responseObject) {
        [weakSelf.tableView.mj_header endRefreshing];
        [self.hudView hideAnimated:YES];
