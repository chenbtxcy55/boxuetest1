...
{
    if (nil == _oldPwdView) {
        _oldPwdView = [[My_TitleAndInput_View alloc]initWithFrame:CGRectMake(0, ScaleW(5), ScreenWidth, ScaleW(95)) title:SSKJLocalized(@"原登录密码", nil) placeHolder:SSKJLocalized(@"请输入原登录密码", nil) keyBoardType:UIKeyboardTypeDefault];
...
{
    if (nil == _pwdView) {
        _pwdView = [[My_TitleAndInput_View alloc]initWithFrame:CGRectMake(0,self.oldPwdView.bottom, ScreenWidth, ScaleW(95)) title:SSKJLocalized(@"新登录密码", nil) placeHolder:SSKJLocalized(@"请输入6-20位字母与数字组合", nil) keyBoardType:UIKeyboardTypeDefault];
...
{
    if (nil == _surePwdView) {
        _surePwdView = [[My_TitleAndInput_View alloc]initWithFrame:CGRectMake(0, self.pwdView.bottom, ScreenWidth, ScaleW(95)) title:SSKJLocalized(@"确认新登录密码", nil) placeHolder:SSKJLocalized(@"确认新登录密码", nil) keyBoardType:UIKeyboardTypeDefault];
...
- (void)requestResetPWD {
    ...
    [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    [[WLHttpManager shareManager]requestWithURL_HTTPCode:kIscm_xiugai_pwd_Api RequestType:RequestTypePost Parameters:params Success:^(NSInteger statusCode, id responseObject) {
        [MBProgressHUD hideHUDForView:weakSelf.view animated:YES];
        WL_Network_Model *network_model = [WL_Network_Model mj_objectWithKeyValues:responseObject];
        if ([network_model.status integerValue] == SUCCESSED) {
            [MBProgressHUD showError:network_model.msg];
