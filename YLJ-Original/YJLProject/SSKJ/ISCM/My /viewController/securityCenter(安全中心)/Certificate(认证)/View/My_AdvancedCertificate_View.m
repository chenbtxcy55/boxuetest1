...
//    高级认证 1 未认证 2 待审核 3 已通过  4拒绝
    switch ([model.auth_status integerValue])
    {
        ...
        {  // 待审核
            self.statusImageView.hidden = YES;
            self.stautsLabel.x = self.titleLabel.x;
            self.stautsLabel.text = SSKJLocalized(@"待审核", nil);
        }
            break;
...
-(UILabel *)infoLabel
{
    if (nil == _infoLabel) {
        ...
        _infoLabel = [FactoryUI createLabelWithFrame:CGRectMake(self.titleLabel.x, self.titleLabel.bottom + ScaleW(14), self.iconImageView.x - self.titleLabel.x, ScaleW(12)) text:string textColor:kTitleGrayColor font:systemFont(ScaleW(11.5))];
...
{
    if (nil == _stautsLabel) {
        _stautsLabel = [FactoryUI createLabelWithFrame:CGRectMake(self.statusImageView.right + ScaleW(5), 0, self.iconImageView.x - self.statusImageView.right - ScaleW(5), ScaleW(12)) text:SSKJLocalized(@"待认证", nil) textColor:kMainBlueColor font:systemFont(ScaleW(11))];
