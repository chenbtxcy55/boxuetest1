...
-(void)uploadImage:(UIImage*)image
{
    ...
    WS(weakSelf);
    [[WLHttpManager shareManager]  upLoadImageByUrl:kIscm_upload_pic_Api ImageName:@"file_pic" Params:nil Image:image CallBack:^(id responseObject)
     {
         ...
         {
             [weakSelf showImage:image withUrl:netModel.data[@"server_url"]];
         }
         else
         {
...
-(void)submitAction:(UIButton*)sender
{
    ...
    {
        NSDictionary *parameters = @{@"cardimg1":self.frontUrl,
                            @"cardimg2":self.backUrl,
                            @"cardimg3":self.handHeldUrl,
                                     };

        [MBProgressHUD showHUDAddedTo:self.view animated:YES];
        WS(weakSelf);
        [[WLHttpManager shareManager] requestWithURL_HTTPCode:kIscm_advance_set_img_Api RequestType:RequestTypePost Parameters:parameters Success:^(NSInteger statusCode, id responseObject)
         {
