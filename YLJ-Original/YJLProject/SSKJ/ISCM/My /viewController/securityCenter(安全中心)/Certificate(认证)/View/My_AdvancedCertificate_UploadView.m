...
-(void)setIdCardType:(IDentityCardType)idCardType
{
    ...
    switch (idCardType)
    {
        ...
        case IDentityCardBack:
        {
            ...
            message = SSKJLocalized(@"身份证反面确保无水印无污渍，身份信息清晰，非文字反向照片，请勿进行PS处理。", nil);
        }
            break;
        case IDentityCardHandHeld:
        {
            imageName = @"Mine_Hand-held";
            title = SSKJLocalized(@"请上手持照片", nil);
            message = SSKJLocalized(@"需要您本人手持您的身份证，确保身份证在您胸前，不遮挡您的脸部，并且身份证的信息清晰可见。", nil);
        }
            break;
    }
...
-(UILabel *)messageLabel
{
    if (nil == _messageLabel)
    {
        ...
        [_messageLabel setTextColor:kTitleGrayColor];
    }
