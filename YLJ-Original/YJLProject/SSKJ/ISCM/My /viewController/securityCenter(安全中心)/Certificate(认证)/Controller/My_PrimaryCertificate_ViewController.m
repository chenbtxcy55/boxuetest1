...
//    [_nameTextField setValue:kTitleGrayColor forKeyPath:@"_placeholderLabel.textColor"];
    ...
    NSMutableAttributedString *placeholderString = [[NSMutableAttributedString alloc] initWithString: @"请输入您的姓名" attributes:@{NSForegroundColorAttributeName : kTitleGrayColor}];
...
//    [_idViewTextField setValue:kTitleGrayColor forKeyPath:@"_placeholderLabel.textColor"];
    ...
    NSMutableAttributedString *placeholderString1 = [[NSMutableAttributedString alloc] initWithString: @"请输入您的身份证号码" attributes:@{NSForegroundColorAttributeName : kTitleGrayColor}];
    
    self.idViewTextField.attributedPlaceholder = placeholderString1;
...
- (void)requestPrimaryCertificate:(NSString*)name withIDentity:(NSString*)identity
{
    ...
    NSDictionary *params = @{@"account":@(accountStr.longLongValue),
                             @"realname":name,
                             @"idcard":identity,
                             @"type":@(1)};
    
    WS(weakSelf);
    [[WLHttpManager shareManager] requestWithURL_HTTPCode:kIscm_primary_set_sm_Api RequestType:RequestTypePost Parameters:params Success:^(NSInteger statusCode, id responseObject)
    {
