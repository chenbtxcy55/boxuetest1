...
//     初级认证 1 未认证 2 待审核 3 已通过  4拒绝
    switch ([userModel.status integerValue])
    {
        ...
        case 3:
        {   // 已认证
            self.infoLabel.text = [NSString stringWithFormat:@"%@ | %@",userModel.name,userModel.idcard];
...
-(UIImageView *)iconImageView
{
    if (nil == _iconImageView)
    {
        ...
        _iconImageView.image = [UIImage imageNamed:@"shenfen_image"];
    }
...
-(UILabel *)infoLabel
{
    if (nil == _infoLabel)
    {
        ...
        [_infoLabel setTextColor:kTitleGrayColor];
        _infoLabel.text = @"孙悟空 | 41*********8521";
...
-(UILabel *)stautsLabel
{
    if (nil == _stautsLabel)
    {
        _stautsLabel = [FactoryUI createLabelWithFrame:CGRectMake(self.statusImageView.right + ScaleW(5), 0, self.iconImageView.x - self.statusImageView.right - ScaleW(5), ScaleW(12)) text:SSKJLocalized(@"认证成功", nil) textColor:kMainBlueColor font:systemFont(ScaleW(11))];
