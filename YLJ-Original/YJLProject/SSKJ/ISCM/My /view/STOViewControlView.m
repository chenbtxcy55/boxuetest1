...
-(instancetype)init{
    if (self = [super init])
    {
        ...
        //self.bttonsArray = @[@"STO账户",@"USDT账户",@"节点账户",@"冻结账户"];
    }
    return self;
}
...
-(void)setBttonsArray:(NSArray *)bttonsArray
{
    ...
    for (int i = 0; i < _bttonsArray.count; i ++) {
        ...
        CGFloat titleWidth = [self returnWidth:titleString font:ScaleW(13)];
        UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
        btn.frame = CGRectMake(ScaleW(15) + allWidth, 0, (titleWidth + ScaleW(15)), ScaleW(40));
      
        [self.contenScrollView addSubview:btn];
        btn.tag = kTag(i);
        [btn btn:btn font:ScaleW(13) textColor:kTitleColor text:SSKJLocalized(_bttonsArray[i], nil) image:nil sel:@selector(bttonsAction:) taget:self];
