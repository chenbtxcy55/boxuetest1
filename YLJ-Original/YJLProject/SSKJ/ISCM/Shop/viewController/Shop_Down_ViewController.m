...
@interface Shop_Down_ViewController ()<UITableViewDataSource,UITableViewDelegate>
@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, strong) NSMutableArray *dataSource;
@property (nonatomic, assign) NSInteger page;
@end

@implementation Shop_Down_ViewController
...
- (void)viewDidLoad {
    ...
    [kNotifyCenter addObserver:self selector:@selector(reloadOther2:) name:@"reloadOther2" object:nil];
    
    [self requesttransferList];
}

-(void)reloadOther2:(NSNotification *)noti
{
...
-(UITableView *)tableView
{
    if (nil == _tableView) {
        ...
        _tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, ScaleW(0), ScreenWidth,ScreenHeight- Height_TabBar -self.bottomLine) style:UITableViewStyleGrouped];
        NSLog(@"height:::%f",ScreenHeight- Height_TabBar -self.bottomLine);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell setCellWithModel:goodsModel goodsType:2];
    WS(weakSelf);
    cell.upGoodsBlock = ^(ICC_PreOrder_GoodsInfo_Model *model) {
        UIAlertController *alertViewControler = [UIAlertController alertControllerWithTitle:@"提示" message:@"确认上架该商品吗" preferredStyle:UIAlertControllerStyleAlert];
...
-(void)requesttransferList
{
    ...
    [[WLHttpManager shareManager]requestWithURL_HTTPCode:VPay_Shop_GoodsList_URL RequestType:RequestTypePost Parameters:params Success:^(NSInteger statusCode, id responseObject) {
        [MBProgressHUD hideHUDForView:weakSelf.view animated:YES];
        WL_Network_Model *net_model = [WL_Network_Model mj_objectWithKeyValues:responseObject];
        if (net_model.status.integerValue == 200) {
            
            [weakSelf handleDataWith:net_model];
            !weakSelf.backBlock?:weakSelf.backBlock([net_model.data[@"off"] integerValue]);
//
        }else{
            [weakSelf endRefresh];
