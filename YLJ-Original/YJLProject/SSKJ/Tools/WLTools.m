...
+(CGFloat)getStringHeight:(NSString *)str fontSize:(float)font witdh:(CGFloat)witdh
{
    NSDictionary *attributes = @{NSFontAttributeName: systemFont(font)};

    CGFloat titleHeight = [str boundingRectWithSize:CGSizeMake(witdh,MAXFLOAT) options: NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil].size.height;
    return titleHeight;
...
+ (NSString*)imageURLWithURL:(NSString *)imageURL
{
    if (![imageURL containsString:@"http"]) {
        if ([imageURL hasPrefix:@"/"]) {
            return [NSString stringWithFormat:@"%@%@",ProductBaseServer,imageURL];
        }else{
            return [NSString stringWithFormat:@"%@/%@",ProductBaseServer,imageURL];
        }
    }
...
+(NSString *)countDownTimeWithTime:(long)time
{
    ...
    if (day == 0) {
        if (houre == 0) {
            timeString = [NSString stringWithFormat:@"%02d分%02d秒",minute,second];
        }else{
            timeString = [NSString stringWithFormat:@"%d时%02d分%02d秒",houre,minute,second];
        }
    }else{
        timeString = [NSString stringWithFormat:@"%d天%d时%02d分%02d秒",day,houre,minute,second];
    }
...
{
    if ([coinName isEqualToString:@"BTC/AB"] || [coinName isEqualToString:@"ETH/AB"] || [coinName isEqualToString:@"BTC/USDT"] || [coinName isEqualToString:@"ETH/USDT"] || [coinName isEqualToString:@"btc_usdt"] || [coinName isEqualToString:@"eth_usdt"]) {
        return 2;
    }else if ([coinName isEqualToString:@"OMG/AB"] || [coinName isEqualToString:@"GNT/AB"] || [coinName isEqualToString:@"PST/AB"] || [coinName isEqualToString:@"ZRX/AB"] || [coinName isEqualToString:@"MANA/AB"] || [coinName isEqualToString:@"CQTF/AB"]){
        return 4;
    }else if ([coinName isEqualToString:@"OMG/BTC"] || [coinName isEqualToString:@"OMG/ETH"]  || [coinName isEqualToString:@"PST/ETH"]){
        return 6;
    }else if ([coinName isEqualToString:@"ZRX/BTC"] || [coinName isEqualToString:@"BAT/BTC"] ||[coinName isEqualToString:@"GNT/BTC"] ||[coinName isEqualToString:@"PST/BTC"] ||[coinName isEqualToString:@"MANA/BTC"]|| [coinName isEqualToString:@"ZRX/ETH"]  || [coinName isEqualToString:@"BAT/ETH"] || [coinName isEqualToString:@"GNT/ETH"] || [coinName isEqualToString:@"MANA/ETH"]){
        return 8;
    }
    return 4;
}
...
{
    if ([coinName isEqualToString:@"PST"] || [coinName isEqualToString:@"ZRX"] || [coinName isEqualToString:@"CQTF"] || [coinName isEqualToString:@"MANA"]) {
        return 2;
    }else if ([coinName isEqualToString:@"BTC"] || [coinName isEqualToString:@"OMG"] || [coinName isEqualToString:@"GNT"]|| [coinName isEqualToString:@"ETH"]){
        return 4;
    }else{
        return 4;
    }
...
+(int)pointCount:(NSString *)code
{
    /*币种
     ...
     dash/usdt  2位
     bch/usdt 2位*/
    if ([code.lowercaseString isEqualToString:@"btc_usdt"] || [code.lowercaseString isEqualToString:@"btc"]) {
        return 2;
    }
    if ([code.lowercaseString isEqualToString:@"eth_usdt"] || [code.lowercaseString isEqualToString:@"eth"]) {
        return 2;
    }
...
+(NSString *)simpleHashWithTel:(NSString *)telNum
{
    if (telNum.length<=4) {
        return telNum;
    }
    NSString *firstString = [telNum substringToIndex:3];
    NSString *lastString = [telNum substringFromIndex:telNum.length - 4];
    return [NSString stringWithFormat:@"%@****%@",firstString,lastString];
}

+(NSString *)simpleHashWithAddress:(NSString *)address
{
    if (address.length<=10) {
        return address;
    }
    NSString *firstString = [address substringToIndex:9];
    NSString *lastString = [address substringFromIndex:address.length - 9];
    return [NSString stringWithFormat:@"%@****%@",firstString,lastString];
}
...
+(BOOL)getDayNight
{
    return ![DayNightHelper getDayNight];
};
...
+ (NSMutableAttributedString *)setUnderLine:(NSString *)str {
    NSMutableAttributedString *attrStr = [[NSMutableAttributedString alloc] initWithString:str
                                                                                attributes:@{NSUnderlineStyleAttributeName : @(NSUnderlineStyleSingle)}];
    return attrStr;
}

+ (NSString *)setTotalPriceWithJifen:(NSString *)jifen andPrice:(NSString *)rmb {
    ...
    dispatch_source_set_event_handler(timer, ^{
        ...
        if(timeout<=0){ //倒计时结束，关闭
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [button setTitle:SSKJLocalized(@"重新发送", nil) forState:UIControlStateNormal];
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                //设置界面的按钮显示 根据自己需求设置
//                [button setTitle:[NSString stringWithFormat:@"%@s%@",strTime,SSKJLocalized(@"重新获取", nil)] forState:UIControlStateDisabled];
                [button setTitle:[NSString stringWithFormat:@"%@(S)%@",strTime,SSKJLocalized(@"", nil)] forState:UIControlStateDisabled];
