...
- (void)startVerifyCompletion:(void (^)(BOOL, NSString * _Nullable, NSString * _Nonnull))completion{
    ...
    if (self.manager) {
        ...
        // 传统验证码
        NSString *captchaid = @"739da606847c437ba13466a79f87a147";
...
- (void)verifyCodeNetError:(NSError *)error{
    //用户关闭验证后执行的方法
  //  WlLog(@"收到网络错误的回调:%@(%ld)", [error localizedDescription], (long)error.code);
}
...
- (void)verifyCodeValidateFinish:(BOOL)result validate:(NSString *)validate message:(NSString *)message{
   // WlLog(@"收到验证结果的回调:(%d,%@,%@)", result, validate, message);
