...
- (void)requestWithURL:(NSString*)URLString RequestType:(RequestType )type Parameters:(NSDictionary*)parameters Success:(void(^)(id responseObject))success Failure:(void(^)(NSError *error,id responseObject))failure{
    ...
    {
        [manager.requestSerializer setValue:@"zh-CN" forHTTPHeaderField:@"locale"];
        ...
        [manager GET_HTTPCode:URLString parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject)
         {
             ...
             if (success)
             {
                 ...
                 if ([network_Model.status integerValue]==404 || [network_Model.status integerValue] == 505)
                 {
...
//        [self.logoutAlertView showWithMessage:SSKJLocalized(@"登录超时，请重新登录", nil)];
//    }
//    SSKJUserDefaultsSET(@"0", @"kLogin");
    [SSKJ_User_Tool clearUserInfo];
    [HeBi_Default_AlertView showWithTitle:SSKJLocalized(@"提示", nil) message:@"登录超时，请重新登录" cancleTitle:SSKJLocalized(@"取消", nil) confirmTitle:SSKJLocalized(@"去登录", nil) confirmBlock:^{
...
-(void)upLoadImageByUrl:(NSString *)URLString ImageName:(NSString*)imageName Params:(NSDictionary *)params Image:(UIImage*)image CallBack:(CallBack)callback Failure:(void(^)(NSError *error))failure{
    ...
    [manager POST:URLString parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData)
    {
        ...
        [formData appendPartWithFileData:imageData name:imageName fileName:[NSString stringWithFormat:@"%@.jpg",[WLTools getOrderTrSeq]] mimeType:@"image/jpeg"];
