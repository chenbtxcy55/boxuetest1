...
@implementation SSKJ_Notice_Cell
...
//        UIImage *image = [UIImage imageNamed:@"gonggaoList"];
...
- (UILabel *)timeLabel{
    if (_timeLabel == nil) {
        _timeLabel = [FactoryUI createLabelWithFrame:CGRectZero text:@"" textColor:kSubTxtColor font:systemFont(12)];
...
- (UIView *)lineView{
    if (_lineView == nil) {
        _lineView = [FactoryUI createViewWithFrame:CGRectZero Color:kLineGrayColor];
...
- (void)initDataWithModel:(SSKJ_NoticeIndex_Model *)Model
{
    self.titleLabel.text = Model.title;
    
//    self.timeLabel.text = [WLTools convertTimestamp:Model.create_time.longLongValue andFormat:@"yyyy-MM-dd HH:mm:ss"];
    self.timeLabel.text = Model.push_time;
}
