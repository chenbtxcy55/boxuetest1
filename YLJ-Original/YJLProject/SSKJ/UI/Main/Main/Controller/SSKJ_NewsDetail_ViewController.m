//
//  SSKJ_NewsDetail_ViewController.m
//  ZYW_MIT
//
...
@interface SSKJ_NewsDetail_ViewController ()<WKUIDelegate,WKNavigationDelegate,NSURLSessionDelegate,UIGestureRecognizerDelegate>
...
@implementation SSKJ_NewsDetail_ViewController
...
- (void)viewWillAppear:(BOOL)animated
{
   ...
   if (self.detailType == DetailTypeNews){
        self.title = SSKJLocalized(@"资讯详情", nil);
//        self.titleLabel.text=self.newsModel.bm_title;
//        self.dataLabel.text = self.newsModel.issue_time;
...
//       self.dataLabel.text = [WLTools convertTimestamp:self.noticeModel.create_time.longLongValue andFormat:@"yyyy-MM-dd HH:mm:ss"] ;
...
-(void)requestNewsDetail
{
    ...
    [[WLHttpManager shareManager]requestWithURL_HTTPCode:kYLJ_Home_zixun_detailApi RequestType:RequestTypePost Parameters:@{@"notice_id":self.noticeModel.ID} Success:^(NSInteger statusCode, id responseObject) {
        
        WL_Network_Model *netmodel = [WL_Network_Model mj_objectWithKeyValues:responseObject];
        
        if (netmodel.status.integerValue == SUCCESSED) {

            self.noticeModel =[SSKJ_NoticeIndex_Model mj_objectWithKeyValues:netmodel.data];
//            self.dataLabel.text = [WLTools convertTimestamp:self.noticeModel.create_time.longLongValue andFormat:@"yyyy-MM-dd HH:mm:ss"] ;
            self.dataLabel.text = self.noticeModel.create_time;

//            [self.webView loadHTMLString:self.noticeModel.content baseURL:[NSURL URLWithString:ProductBaseServer]];
            [self htmlStringWithString:self.noticeModel.content];
...
-(UIView *)titleBackView
{
    if (nil == _titleBackView) {
        ...
        _titleBackView.backgroundColor = kMainColor;
...
- (UILabel *)titleLabel
{
    if (!_titleLabel)
    {
        ...
        _titleLabel.font=[UIFont systemFontOfSize:ScaleW(18)];
...
- (UILabel *)dataLabel
{
    if (!_dataLabel)
    {
        ...
        _dataLabel.font=[UIFont systemFontOfSize:ScaleW(15)];
        
        _dataLabel.textColor=kSubTxtColor;
...
//        _lineView.backgroundColor = kTextGrayColor;
...
//        _webBackView.backgroundColor = kNavBGColor;
        [self.view addSubview:_webBackView];
        
        [_webBackView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.titleBackView.mas_bottom).offset(ScaleW(1));
...
- (void)webView:(WKWebView *)webView didStartProvisionalNavigation:(WKNavigation *)navigation
{
    ...
    //修改字体颜色  #9098b8
    [webView evaluateJavaScript:@"document.getElementsByTagName('body')[0].style.webkitTextFillColor= '#6c6c6c'"completionHandler:nil];
