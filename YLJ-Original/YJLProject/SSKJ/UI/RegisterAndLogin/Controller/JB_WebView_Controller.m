...
- (void)viewDidLoad {
    ...
    if (self.protocolType == PROTOCOLTYPEPABOUT) {
        self.url =  kIscm_user_zixun_detailApi;
    }
    ...
    if (_nmodel) {
        ...
        self.titleLabel.text = _nmodel.title;
        self.dateLabel.text = _nmodel.push_time;
        NSString *detailTextString = _nmodel.content;
    ...
    if (self.protocolType == PROTOCOLTYPEREGISTER) {
        self.url =  kIscm_agree_Api;
...
-(UIScrollView *)contenScrollView
{
    if (!_contenScrollView) {
        ...
        _contenScrollView.backgroundColor = kMainColor;
...
-(UILabel *)dateLabel{
    if (!_dateLabel ) {
        ...
        [_dateLabel label:_dateLabel font:ScaleW(12) textColor:kMainTextColor text:@""];
...
-(void)requstAcction
{
    ...
    [[WLHttpManager shareManager]requestWithURL_HTTPCode:kIscm_user_zixun_detailApi RequestType:RequestTypeGet Parameters:params Success:^(NSInteger statusCode, id responseObject) {
        WL_Network_Model * netWorkModel = [WL_Network_Model mj_objectWithKeyValues:responseObject];
        [MBProgressHUD hideHUDForView:weakSelf.view animated:YES];
        if ([netWorkModel.status isEqualToString:@"200"]) {
            
            self.titleLabel.text = netWorkModel.data[@"title"];
            self.dateLabel.text = [WLTools convertTimestamp:[netWorkModel.data[@"create_time"]  doubleValue]andFormat:@"yyyy-MM-dd HH:mm:ss"];
...
//                key = @"reg_agree";
                weakSelf.htmlString = network_model.data;
                [self htmlStringWithString:self.htmlString];
                return ;
