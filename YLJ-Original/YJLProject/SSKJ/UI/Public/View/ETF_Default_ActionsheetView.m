...
+(void)showWithItems:(NSArray *)itemsArray title:(NSString *)title selectedIndexBlock:(void(^)(NSInteger selectIndex))selectIndexBlcok cancleBlock:(void(^)(void))cancleBlock
{
    ...
    actionSheet.title = [NSString stringWithFormat:@"%@",title];
...
-(UIView *)showView
{
    if (nil == _showView) {
        ...
        _showView.backgroundColor = kBgColor353750;
    }
    return _showView;
}
...
-(UIButton *)cancleButton
{
    if (nil == _cancleButton) {
        ...
        [_cancleButton setTitleColor:kMainRedColor forState:UIControlStateNormal];
        ...
        _cancleButton.backgroundColor = kBgColor353750;
    }
...
{
    if (nil == _titleLabel) {
        _titleLabel = [WLTools allocLabel:@"" font:systemBoldFont(ScaleW(14)) textColor:kMainTextColor frame:CGRectMake(0, 0, ScreenWidth, ScaleW(48)) textAlignment:NSTextAlignmentCenter];
        _titleLabel.backgroundColor = kBgColor353750;
        UIView *lineView = [[UIView alloc]initWithFrame:CGRectMake(0, _titleLabel.height-.5, _titleLabel.width, 0.5)];
...
-(void)setItemsArray:(NSArray *)itemsArray
{
    ...
    for (int i = 0; i < self.itemsArray.count; i++) {
        ...
        UIView *lineView = [[UIView alloc]initWithFrame:CGRectMake(0, btn.height-.5, btn.width, 0.5)];
        lineView.backgroundColor = UIColorFromRGB(0x4c4e6b);
    ...
    self.cancleButton.y = startY + self.itemsArray.count * ScaleW(48) + ScaleW(10);
