...
{
    if (nil == _titlesArray) {
        _titlesArray  = @[SSKJLocalized(@"时间", nil),SSKJLocalized(@"最高", nil),SSKJLocalized(@"最低", nil),SSKJLocalized(@"开盘", nil),SSKJLocalized(@"收盘", nil),SSKJLocalized(@"成交量", nil)];
    }
    return _titlesArray;
}
...
//                [WLTools noroundingStringWith:model.close.doubleValue afterPointNumber:LXY_DecimalNumber];
        ...
        UILabel *label = [WLTools allocLabel:[NSString stringWithFormat:@"%@:%@",title,value] font:systemFont(10) textColor:color frame:CGRectMake(0, 12 * i, self.width, 10) textAlignment:NSTextAlignmentLeft];
