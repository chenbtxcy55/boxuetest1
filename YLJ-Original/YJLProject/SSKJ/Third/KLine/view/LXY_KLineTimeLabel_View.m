...
//        count =  (self.countOfShowCandle / (LXY_NumberofTimeLine + aa));
        ...
        if (i == LXY_NumberofTimeLine + 1) {
            ...
            if (self.startDrawIndex + (NSInteger)(count * i)>=self.dataSource.count) {
                
                return;
            }
            model = self.dataSource[self.startDrawIndex + (NSInteger)(count * i)];
...
-(void)drawLongPressTime
{
    ...
    [[UIColor whiteColor]setStroke];
    UIFont  *font = [UIFont boldSystemFontOfSize:10];//设置字体
    NSDictionary *attribute = @{NSFontAttributeName:font,NSForegroundColorAttributeName:[UIColor lxy_priceColor]};
