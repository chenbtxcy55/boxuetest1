...
-(void)drawLine
{
    ...
    for (int i = 0; i < LXY_NumberofPriceline + 2; i++) {
        ...
        [self drawAvgLineWith:context movePoint:CGPointMake(0, 10 + i * lineGap) stopPoint:CGPointMake(self.width, 10 + i * lineGap) lineColor:[UIColor lxy_kLine_Divider_Color]];
    ...
    for (int i = 0; i < LXY_NumberofTimeLine + 2; i++) {
        ...
        [self drawAvgLineWith:context movePoint:CGPointMake(lineGap * i, 0) stopPoint:CGPointMake(lineGap * i, self.height) lineColor:[UIColor lxy_kLine_Divider_Color]];
...
-(void)drawMaxAndMin
{
    ...
    LXY_KLine_DataModel *minPriceModel =[LXY_KLine_DataModel new];
    
    
    if (self.minPriceIndex <self.dataSource.count) {
        
        minPriceModel=self.dataSource[self.minPriceIndex];
    }
    // 画最大值
